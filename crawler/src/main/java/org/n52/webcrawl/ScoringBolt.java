...
public class ScoringBolt extends BaseBasicBolt {
    ...
    public void execute(Tuple input, BasicOutputCollector collector) {
        ...
        m.setValue("score", Float.toString(score));

        List<String> keywords = generateKeywords(m);
        m.setValues("keywords", keywords.toArray(new String[keywords.size()]));
    ...
    private List<String> generateKeywords (Metadata m) {
        ...
        if (m.getFirstValue("extracted.data_portal") != null) keywords.add("dataportal");
        if (m.getFirstValue("extracted.data_api") != null)    keywords.add("api");
        if (m.getFirstValue("extracted.data_link") != null)   keywords.add("datalink");
        if (m.getFirstValue("extracted.data_pdf") != null)   keywords.add("pdf");

        String keyword;
        keyword = m.getFirstValue("keywords.dataset");
        if (keyword != null && !keyword.equals("0.0")) keywords.add("dataset");
        keyword = m.getFirstValue("keywords.historic");
        if (keyword != null && !keyword.equals("0.0")) keywords.add("historic");
        keyword = m.getFirstValue("keywords.realtime");
    ...
    private float calcScore (Metadata m) {
        ...
        String clazz = m.getFirstValue("classify.class");
        String confidence = m.getFirstValue("classify.confidence");
        ...
        // from SimpleTopicScoreParseFilter
        String keyword;
        keyword = m.getFirstValue("keywords.historic");
        ...
        // from XPath3Filter
        String[] extractlist;
        extractlist = m.getValues("extracted.data_api");
        if (extractlist != null) score += weightExtract * Math.max(extractlist.length, 2);
        extractlist = m.getValues("extracted.data_link");
        if (extractlist != null) score += weightExtract * Math.max(extractlist.length, 2);
        extractlist = m.getValues("extracted.data_portal");
        if (extractlist != null) score += weightExtract * Math.max(extractlist.length, 2);
        extractlist = m.getValues("extracted.license");
        if (extractlist != null) score += weightExtract * Math.max(extractlist.length, 2);
